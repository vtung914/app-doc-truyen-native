...
class MainActivity : AppCompatActivity(), TextToSpeech.OnInitListener {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        // Tạo giao diện nhanh bằng Code (Không cần file xml phức tạp)
        ...
        // Khởi tạo bộ đọc và thư viện PDF
    ...
    override fun onInit(status: Int) {
        if (status == TextToSpeech.SUCCESS) {
            tts.language = Locale("vi", "VN") // Đặt giọng đọc tiếng Việt
            setupTTSListener()
        }
    }
    ...
    private fun processTextIntoSentences(text: String) {
        ...
        // Tách câu theo các dấu ngắt câu cơ bản
    ...
    private fun setupTTSListener() {
        tts.setOnUtteranceProgressListener(object : UtteranceProgressListener() {
            ...
            override fun onDone(utteranceId: String?) {
                val index = utteranceId?.toIntOrNull() ?: return
                speakFromIndex(index + 1) // Tự động chuyển sang câu tiếp theo khi đọc xong
            }
    ...
    private fun highlightSentence(index: Int) {
        ...
        // Bôi màu nền vàng đoạn đang đọc
        spannable.setSpan(
        ...
        // Tự động cuộn màn hình theo câu đang đọc nếu văn bản quá dài
